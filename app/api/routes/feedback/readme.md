...
- находит бронирования, завершённые **более 15 минут назад**
- проверяет, что запрос на отзыв ещё не был отправлен
- инициирует отправку запроса на отзыв
- помечает бронирование как обработанное

Логика расположена в:

```text
app/domain/services/feedback/
├── reminder.py   # бизнес-логика
└── tasks.py      # запуск фоновой задачи
